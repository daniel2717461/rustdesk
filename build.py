...
hbb_name = '微益智' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/微益智.exe')
    os.chdir('../..')
    if os.path.exists('./微益智_portable.exe'):
        os.replace('./target/release/微益智-portable-packer.exe',
                   './微益智_portable.exe')
    else:
        os.rename('./target/release/微益智k-portable-packer.exe',
                  './微益智_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/微益智_portable.exe')
    os.rename('./微益智portable.exe', f'./微益智-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/微益智-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/微益智.exe target/release/微益智.exe')
        pa = os.environ.get('P')
        if pa:
